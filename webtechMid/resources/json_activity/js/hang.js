...
function rounds() {
  ...
  if(alpha.toString() == fillMe.toString()) { 
	...
	setElementAttr(nextBtn,{"value": "Restart!"})
	//quest();
  } else if (livesLeft===0) { 
...
function quest(){
	if(startQuest!=maxQuest){ //checks if currentQuest is not equal to maxQuest
		...
		start(); //next question for the game
...
function isEligible(){ //checks 
	...
	if(wrongs == 0){ 
		totalScore = 100;
		document.getElementById("nextLesson").style.visibility="visible"; //sets the next lesson visible 
	}else if(totalScore >= 60){
		document.getElementById("nextLesson").style.visibility="visible"; 
